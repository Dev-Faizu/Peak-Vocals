...
class UVRWebUI:
    ...
    def define_layout(self):
        with gr.Blocks() as app:
            self.app = app
            gr.HTML("<h1> Ƥᴇʌᴋ V๏ᴄʌʟꜱ - Dev Faiz </h1>")
            gr.Markdown("Dont use these space for Private use")
            gr.Markdown(
                "[![Follow me on instagram](https://huggingface.co/datasets/huggingface/badges/resolve/main/follow-us-on-hf-sm-dark.svg)](https://instagram.com/master_faiz)\n\n"
            ) 
            with gr.Tabs():
                with gr.TabItem("process"):
                    with gr.Row():
                        ...
                        self.model_choice = gr.Dropdown(
                            ...
                            label=SELECT_VR_MODEL_MAIN_LABEL+' 👋Choose a model', interactive=True)
                    with gr.Row():
                        self.arch_setting1 = gr.Dropdown(
                            choices=VR_WINDOW, value=root.window_size_var.get(),
                            label=WINDOW_SIZE_MAIN_LABEL+' 👋Choose one', interactive=True)
                    ...
                    with gr.Row():
                        self.input_filename = gr.Textbox(label="Input filename", value="Peaked_audio.wav", interactive=True)
                    with gr.Row():
